...
function ProductCard({ product, onAddToCart, onUpdatePrice, onUpdateProduct, onDeleteProduct, categories }) {
  ...
  return (
    <>
    <div className="group bg-white rounded-lg shadow-md hover:shadow-lg p-1.5 md:p-2.5 flex flex-col justify-between transition-all duration-300 hover:-translate-y-0.5 border border-gray-100 relative overflow-hidden">
      ...
      <button
        ...
        className="absolute top-1 right-1 md:top-2 md:right-2 p-1 md:p-2 bg-emerald-500 text-white hover:bg-emerald-600 rounded-lg transition-all duration-200 z-20 shadow-lg hover:shadow-xl border-2 border-emerald-600 hover:scale-110"
        title="Edit product"
      >
      ...
      <div className="flex-1 mb-2 relative z-10">
        {/* Product Image */}
        <div className="mb-1 md:mb-2 rounded-md overflow-hidden bg-transparent aspect-square flex items-center justify-center group-hover:scale-105 transition-transform duration-300 min-h-[80px] md:min-h-[120px] relative">
          {!imageError && currentImageSrc ? (
            ...
            <div className="w-full h-full flex items-center justify-center bg-gray-100">
              <span className="text-gray-700 text-[10px] md:text-xs font-bold text-center px-1 md:px-2">
                {product.name}
              </span>
            </div>
          )}
        ...
        <div className="flex items-center justify-between gap-1 md:gap-2 mb-1 md:mb-2">
          <h3 className="text-[10px] md:text-sm font-bold text-gray-900 leading-tight group-hover:text-emerald-700 transition-colors flex-1 truncate">
            {product.name}
          </h3>
          <span className={`px-1 md:px-1.5 py-0.5 rounded text-[7px] md:text-[9px] font-bold uppercase tracking-wide border flex-shrink-0 ${categoryColor}`}>
            {product.category}
          </span>
        </div>
        
        <div className="mb-1 md:mb-2">
          <p className="text-[8px] md:text-[9px] text-gray-500 font-medium mb-0.5">Price</p>
          <div className="flex items-center justify-between gap-1 md:gap-2">
            <p className="text-sm md:text-xl font-black text-emerald-600 leading-none">
              ₱{product.price.toFixed(2)}
            </p>
            {/* Quantity Selector */}
            <div className="flex items-center gap-0.5 md:gap-1.5 bg-gray-50 rounded-lg border border-gray-200 p-0.5 md:p-1">
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  setQuantity(prev => Math.max(1, prev - 1));
                }}
                className="w-4 h-4 md:w-6 md:h-6 flex items-center justify-center bg-white text-emerald-600 rounded hover:bg-emerald-50 transition-colors font-black text-xs md:text-sm disabled:opacity-30 disabled:cursor-not-allowed"
                disabled={quantity <= 1}
              >
                −
              </button>
              <span className="min-w-[1rem] md:min-w-[1.5rem] text-center text-xs md:text-sm font-bold text-gray-900">
                {quantity}
              </span>
              <button
                onClick={(e) => {
                ...
                className="w-4 h-4 md:w-6 md:h-6 flex items-center justify-center bg-white text-emerald-600 rounded hover:bg-emerald-50 transition-colors font-black text-xs md:text-sm"
              >
                +
              </button>
            </div>
      ...
      <button
        ...
        className={`w-full bg-gradient-to-r from-emerald-500 to-emerald-600 text-white py-1.5 md:py-2 px-1 md:px-2 rounded-md font-bold hover:from-emerald-600 hover:to-emerald-700 transition-all duration-200 flex items-center justify-center gap-0.5 md:gap-1 disabled:opacity-50 disabled:cursor-not-allowed shadow-md hover:shadow-lg relative z-10 group/btn text-[10px] md:text-xs overflow-hidden ${
      ...
      >
        {isAdding ? (
          ...
          <>
            <span className="text-sm md:text-base font-bold group-hover/btn:scale-110 transition-transform">+</span>
